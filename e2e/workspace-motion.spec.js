...
test('首屏载入后可切换菜单并开关命令面板', async ({ page }) => {
  ...
  await page.goto('/#/')
  await expect(page.getByTestId('boot-layer')).toHaveCount(0)
...
test('减少动效时关闭进度、环境光和位移动画', async ({ page }) => {
  ...
  await page.goto('/#/')
  const motionToggle = page.getByRole('button', { name: '系统设置已减少动效' })
...
test('运行中跟随系统动效变化，同时保留手动关闭偏好', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await page.goto('/#/')
  await expect(page.getByRole('button', { name: '系统设置已减少动效' })).toBeDisabled()
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await expect(page.getByRole('button', { name: '关闭动效', exact: true })).toBeEnabled()
  await expect(page.locator('canvas.tech-field')).toBeVisible()
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await expect(page.getByRole('button', { name: '系统设置已减少动效' })).toHaveAttribute('aria-pressed', 'false')
  await page.getByRole('button', { name: '我的工作区', exact: true }).click()
  expect(await page.locator('.home-view-indicator').evaluate(node => node.getAnimations().filter(animation => animation.playState === 'running').length)).toBe(0)
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await page.getByRole('button', { name: '关闭动效', exact: true }).click()
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await expect(page.getByRole('button', { name: '系统设置已减少动效' })).toBeDisabled()
  await page.emulateMedia({ reducedMotion: 'no-preference' })
  await expect(page.getByRole('button', { name: '开启动效', exact: true })).toHaveAttribute('aria-pressed', 'false')
  await expect(page.locator('html')).toHaveAttribute('data-motion', 'off')
  await expect(page.locator('canvas.tech-field')).toHaveCount(0)
})
