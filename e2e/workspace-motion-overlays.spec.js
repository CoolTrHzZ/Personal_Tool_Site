...
test('全屏工具上方可操作命令面板，Escape 逐层退出', async ({ page }) => {
  ...
  const dialog = page.getByRole('dialog', { name: '命令面板' })
  await expect(dialog).toBeVisible()
  await expect.poll(() => fullscreen.evaluate(node => Boolean(node.closest('[inert]')))).toBe(true)
  ...
  await expect(dialog).toHaveCount(0)
  await expect(fullscreen).toBeVisible()
  await expect.poll(() => fullscreen.evaluate(node => Boolean(node.closest('[inert]')))).toBe(false)
  await expect(trigger).toBeFocused()
  await page.keyboard.press('Escape')
