...
export default function CfgLibraryPage() {
  ...
  const setFilter = (key: string, value: string) => setParams(current => { const next = new URLSearchParams(current); if (value && !(key === 'sort' && value === 'default')) next.set(key, value); else next.delete(key); return next }, { replace: true })
