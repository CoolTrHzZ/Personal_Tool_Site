...
export default function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const items = [...toolHits.map(tool => ({ run: () => { addRecentTool(tool.id); navigate(tool.path) } })), ...siteHits.map(item => ({ run: () => { window.open(item.url, '_blank', 'noopener,noreferrer') } }))]
  ...
  return (
    <Modal open={open} title="搜索" onClose={onClose} className="command-palette" hideActions>
      <div className="command-palette-search">
        <Search size={16} />
        <Input ref={inputRef} value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索工具、网站、标签" aria-label="命令面板搜索" onKeyDown={event => {
          if (event.key === 'ArrowDown') { event.preventDefault(); setIndex(current => Math.min(items.length - 1, current + 1)) }
      ...
      <div className="command-list" role="listbox">
        ...
        {toolHits.map((tool, offset) => <button type="button" key={tool.id} className="command-item" role="option" aria-selected={index === offset} onClick={() => run(offset)}><span>{tool.name}</span><small>↵</small></button>)}
