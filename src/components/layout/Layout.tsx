...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => mountTechField(shellRef.current), [])
  const value = useMemo(() => ({ query, setQuery, openPalette: () => setPaletteOpen(true) }), [query])
  return (
    <SearchContext.Provider value={value}>
      <div ref={shellRef} className="app-shell carbon-fx" data-scene={sceneFromPath(pathname)} onPointerMove={onCarbonPointer}>
        <BootLayer />
        <Header />
